...
from optim_attack import dlg_algorithm, stg_algorithm, ig_algorithm,zgla_algorithm
...
def parse_args():
    ...
    parser.add_argument("--max_iteration", help="iteration to reconstruct", default=5000, type=int)
    ...
    parser.add_argument("--gz", help="grad type used fpr experiments", default="false", type=bool)
    parser.add_argument("--zgla_use", help="zgla use autograd or zoograd", default="false", type=bool)
...
if __name__ == "__main__":
    ...
    if args.algorithm not in ["fgla", "dlg", "stg", "ig", "fgla_stg"]:
    ...
    grad_dl = get_grad_dl(resnet50, dataloader, device=args.device, seed=seed,usegz=args.gz)
    ...
    for x, y, grad, mean_var_list in grad_dl:
        ...
        if args.algorithm == "fgla":
            ...
            # dummy_x = stg_algorithm(grad, y, mean_var_list, resnet50, (args.batch_size, 3, 224, 224),
            #                         args.max_iteration,
            #                         args.device, seed=seed, is_zoo=False)
            ...
            decoder.load_state_dict(torch.load(args.model_weights))
            dummy_x = gen_attack_algorithm(grad, y, decoder, True, args.device)
            dummy_x = zgla_algorithm(grad, y, mean_var_list, resnet50, (args.batch_size, 3, 224, 224),
                                    args.max_iteration,
                                    args.device, seed, dummy_x=dummy_x,is_zoo=args.zgla_use)
